...
- oracle7，centos7 (都需要在GRUB中启用CGroupV1否则无法启动，详细介绍见常见问题答疑)
- kali，archlinux
...
CPU核数需小于或等于宿主机核数；内存单位为 MB，硬盘单位为 GB，下载/上传速度单位为 Mbit。是否启用 IPv6 可填 Y 或 N，也可留空（默认不开启）。

如果```外网起端口```和```外网止端口```都设置为 0，则不做区间外网端口映射，只映射基础 SSH 端口。注意这两个参数不能为空，不进行映射时需填写 0。
