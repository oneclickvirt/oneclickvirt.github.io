...
若宿主机只有 SLAAC 分配的 IPv6 `/64`，该前缀是宿主机的已连接路由，不能再切成 PVE/IPAM 子网。默认 IPv6 NAT 使用与宿主路由不重叠的 ULA 网段和 NAT66；只有上游明确路由/委派了额外前缀或地址、且 NDP 可用时，虚拟机或容器才可能获得可从互联网直达的独立公网 `/128`。安装提示中的范围选项不会把 SLAAC `/64` 自动变成公网地址池。
