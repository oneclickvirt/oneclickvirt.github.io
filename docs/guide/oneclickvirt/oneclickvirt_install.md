...
受控端需要先安装并初始化对应的虚拟化环境。SSH 模式需要面板可达节点的 SSH 地址；Agent 模式需要在节点执行面板生成的 Agent 安装命令，并确保 Agent 可达面板的 WebSocket 地址。流量监控也需要 Agent 正常工作。交互与无人值守入口、统一环境变量及网络验收方法见[节点环境的交互与自动化](./environment_modes)。
