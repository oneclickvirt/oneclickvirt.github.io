...
启动流量监控会有对应的内存负担，因为需要缓存流量统计数据，哪怕极致优化后，还是会随着带宽的增大而增大，内存不够用的不要开启流量统计。
...
| 带宽 | 实例的带宽范围 | `plugin_pipe_size` | `sql_cache_entries` | 预估基础内存占用 | 预估突发内存占用 |
|---:|:---:|:---:|:---:|:---:|:---|
| 低带宽 | ≤50 Mbps | 2 MB | 32 | 5~8 MB | 8~12 MB |
| 中低带宽 | 51~100 Mbps | 10 MB | 64 | 10~15 MB | 15~30 MB |
| 中等带宽 | 101~200 Mbps | 25 MB | 128 | 15~20 MB | 30~50 MB |
| 中高带宽 | 201~500 Mbps | 50 MB | 256 | 20~30 MB | 50~80 MB |
| 高带宽 | 501~1000 Mbps | 128 MB | 512 | 30~50 MB | 80~150 MB |
| 超高带宽 | 1001~2000 Mbps | 256 MB | 768 | 50~80 MB | 150~300 MB |
| 极高带宽 | >2000 Mbps | 512 MB | 1024 | 80~120 MB | 300~600 MB |

流量统计的误差10G大概会偏差0.1G左右，已自动处理NAT映射以及带IPV6独立IP的情况，无需手动管理了。但如果是之前有启用了流量统计，后续需要关闭并卸载监控了，那么需要在下面这个地方先对应删除了监控再关闭流量统计，这样才能确保完全卸载，否则还是会在后台统计。
