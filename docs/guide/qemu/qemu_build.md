...
- 可启用虚拟机 IPv6 出站网络（需安装脚本已配置 IPv6 NAT 网络）
- 支持 x86_64 和 ARM64 架构的服务器

:::tip IPv6 网络模式
宿主机只有 SLAAC 分配的公网 `/64` 时，QEMU 使用与宿主路由不重叠的 ULA 网段和 NAT66。参数 `independent_ipv6=y` 在此模式下仅启用虚拟机 IPv6 出站，不会分配可从互联网直接访问的公网 `/128`。
:::
