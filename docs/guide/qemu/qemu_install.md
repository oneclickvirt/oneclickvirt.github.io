...
```shell
bash <(wget -qO- https://cdn.spiritlhl.net/https://raw.githubusercontent.com/oneclickvirt/qemu/main/qemuinstall.sh)
```
...
:::warning
脚本会在执行前要求输入 `yes` 确认，操作不可逆。删除内容包括所有虚拟机、镜像及 QEMU/KVM 网络配置。
