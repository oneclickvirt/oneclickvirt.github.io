...
- 可配置路由公网 IPv6 地址（需安装脚本已配置 `podman-ipv6` 网络）
- 支持 x86_64 和 ARM64 架构的服务器

:::tip IPv6 网络模式
`podman-ipv6` 使用受控 ULA 网桥；独立公网地址会以路由 `/128` 和 NDP 方式接入，而不会把宿主机的公网 `/64` 切成 Netavark IPAM 子网。宿主机需有可用公网 IPv6，且上游必须允许相应的 NDP 邻居响应。
:::
