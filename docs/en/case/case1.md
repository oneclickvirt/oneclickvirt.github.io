...
- Private carrier speed nodes come from `privatespeedtest v0.0.13`: the default path immediately uses the release-validated embedded registry instead of waiting for a remote registry before a user test. A lightweight HTTP/HTTPS request is the preferred reachability and latency signal, TCP connect/TCPing is second, and ICMP is a last fallback; ICMP filtering never removes an HTTP- or TCP-reachable node. A collector-side TCP failure retains a carrier address as a candidate rather than deleting it. After the user host performs its HTTP/TCP/ICMP confirmation, selection ranks probe evidence and latency, then chooses city-diverse primary and bounded fallback nodes from the complete carrier candidate set. Fallback candidates are tried only until the requested number of usable throughput results is reached. A result with either valid upload or download data is shown rather than being silently treated as a failure; an upstream's explicit unavailable status remains respected.
