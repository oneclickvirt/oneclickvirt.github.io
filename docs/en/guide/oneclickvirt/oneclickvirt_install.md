...
A controlled node needs an installed and initialized virtualization runtime. SSH mode requires the panel to reach the node's SSH endpoint. Agent mode requires running the Agent installation command generated by the panel and allowing the Agent to reach the panel's WebSocket endpoint. Traffic monitoring also requires a working Agent. See [Interactive and Unattended Node Setup](./environment_modes) for the common environment flag, entry points and network checks.
